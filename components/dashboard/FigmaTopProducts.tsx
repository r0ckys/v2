...
const FigmaTopProducts: React.FC<FigmaTopProductsProps> = ({
  ...
  return (
    <div className="w-full h-full px-4 py-5 bg-white rounded-xl border border-zinc-200 flex flex-col justify-start items-start gap-4 overflow-hidden">
      {/* Header */}
        ...
        <div className="w-full p-2 bg-neutral-50 rounded-lg outline outline-1 outline-offset-[-1px] outline-neutral-200 flex justify-start items-center gap-1.5 overflow-hidden">
          <div className="flex justify-start items-center gap-2 flex-1">
            ...
            <input
              ...
              className="flex-1 bg-transparent border-none outline-none text-neutral-700 text-xs sm:text-sm font-normal font-['Lato'] placeholder:text-neutral-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="w-full flex-1 flex flex-col justify-start items-start gap-3 sm:gap-4 overflow-y-auto pr-1 sm:pr-2">
        {filteredProducts.length === 0 ? (
          <div className="w-full py-8 text-center text-neutral-400 text-sm">No products found</div>
        ) : (
          filteredProducts.map((product) => (
            <div 
              key={product.id} 
              className="w-full pr-1 sm:pr-2 pb-2 border-b border-neutral-300 flex justify-start items-center gap-2 sm:gap-4 hover:bg-gray-50 transition-colors cursor-pointer rounded"
